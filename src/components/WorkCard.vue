<template>
  <div>
    <v-card class="mb-3 black--text" v-for="(item, index) in cards" :key="index">
      ...
      <v-card-actions>
        <v-spacer></v-spacer>
        <v-btn flat color="primary" :disabled="item.btnDemo.disabled" :href="item.btnDemo.href" :target="item.btnDemo.target">Demo</v-btn>
        <v-btn flat color="primary" :disabled="item.btnGitHub.disabled" :href="item.btnGitHub.href" target="_blank">GitHub</v-btn>
      </v-card-actions>
    </v-card>
  </div>
</template>
